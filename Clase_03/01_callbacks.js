...
const evaluarParesMap = valoresOriginales.map(suma);
...
// const miFuncionMap = (arreglo, callback, string) => {
//     let nuevoArreglo = [];
//     console.log(string);
//     for (let i = 0; i < arreglo.length; i++) {
//         let nuevoValor = callback(arreglo[i]);
//         // let nuevoValor = x => x * 2;
...
Array.prototype.miFuncionMap = function (callback) {
    ...
    for (let i = 0; i < this.length; i++) {
        ...
        nuevoArreglo.push(`${nuevoValor} - miMap`);
    }
    return nuevoArreglo;
}
// console.log(arregloDePrueba.miFuncionMap(x => x * 2));
// arregloDePrueba.miFuncionMap(x => x * 2)
// arregloDePrueba.map(x => x * 2);
// console.log(nuevoArregloConMap);
